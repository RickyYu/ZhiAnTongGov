...
public class AppTools {
    ...
    ///获取区域code对应的中文名称
    class func getAreaCodeByName(code: String) -> String {
        let area = getArea()
        let name = area.getCode2CN(code);
        guard !name.isEmpty else {
            return code
        }
        return area.getCode2CN(code)
    }
    
    ///获取区域对象
    class func getArea() -> Area {
        return self.loadNSUserDefaultsClassValue("area") as! Area
    }
